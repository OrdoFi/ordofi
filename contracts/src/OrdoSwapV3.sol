...
///
/// Two guards, after a researcher showed the float could be drained. v2's leg
/// runner passed a bare amount from one leg to the next and trusted each leg's
/// *declared* input token: swap dust into a junk token you priced yourself so
/// the first leg returns a large number, declare WETH as the second leg's
/// input, and the contract swapped that number of WETH out of its own float
/// and paid you the proceeds. Nothing on the user path compared the float
/// before and after; only the reclaim did.
///
///   1. Continuity: leg i+1 must consume exactly the asset leg i produced
///      (ether as WETH or native being the same asset). `LegMismatch` otherwise.
///   2. The user path may not touch the float: WETH plus ether held before the
///      swap must still be there after the user has been paid, before the
///      reclaim runs. `FloatTouched` otherwise.
///
/// Either alone stops the drain; both are cheap; the second also catches
/// whatever the first did not think of.
///
...
contract OrdoSwapV3 is V4Swap {
    ...
    /// @notice The user's swap would have left the float smaller than it found it.
    ...
    function swap(
        ...
        // The float, before the user's route runs: everything in ether that is
        // not the ether the user just sent. Checked again once the user and the
        // treasury have been paid; the route may not have spent any of it.
        uint256 floatBefore = WETH.balanceOf(address(this)) + address(this).balance - msg.value;
        ...
        uint256 floatAfter = WETH.balanceOf(address(this)) + address(this).balance;
        if (floatAfter < floatBefore) revert FloatTouched(floatBefore, floatAfter);
    ...
    ///
    ///      Each leg must consume what the previous one produced. Without this,
    ///      a leg could declare any input it liked and the runner would spend
    ///      that much of it from the contract's own balance — which is how the
    ///      float was drainable in v2.
    function _run(Leg[] calldata legs, uint256 amount, bool haveNative) private returns (uint256, bool) {
        // What we are holding, named as an ERC-20 (ether is WETH either way).
        (address t0, bool n0) = _inputOf(legs[0]);
        address holding = _asErc20(t0, n0);
        for (uint256 i = 0; i < legs.length; i++) {
            Leg calldata l = legs[i];
            (address td, bool nd) = _inputOf(l);
            address declared = _asErc20(td, nd);
            ...
            address produced = _outputOf(l);
            holding = produced == V4Actions.NATIVE ? address(WETH) : produced;
        }
        return (amount, haveNative);
    }

    /// @dev The asset of an `_inputOf` answer, with native ether named as WETH.
    function _asErc20(address token, bool isNative) private view returns (address) {
        return isNative || token == V4Actions.NATIVE ? address(WETH) : token;
    }
